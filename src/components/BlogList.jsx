            ...
            <AnimatePresence mode="popLayout">
              {filteredArticles.map((article, index) => (
                ...
                >
                  <Link
                    to={`/blog/${article.id}`}
                    className="block p-6 rounded-2xl bg-muted/30 hover:bg-muted/50 border border-transparent hover:border-accent/20 transition-all group"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <h2 className="text-lg font-bold text-primary group-hover:text-accent transition-colors mb-2 truncate">
                          {article.title}
                        </h2>
